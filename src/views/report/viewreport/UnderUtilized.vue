<template>
  <CCard>
    <CCardHeader> Underutilized Vehicle </CCardHeader>
    <CCardBody>
      <CDataTable
        :items="items"
        :fields="fields"
...
<script>
import DashboardService from "@/service/AnalyticsService";
...
export default {
  ...
  methods: {
    underutilized() {
      DashboardService.getAllUnderutilizedVehicle()
        .then((response) => {
          console.log("Coming response" + response.status);
          if (response.status === 200) {
            let serverData = response.data;
            this.items = serverData.vehicleList;
